...
#0602mask_part_face_swapping.py
...
output_file = "0803_compare\\output" # output_filename
source_path = "0803_compare\\unmasked"
target_path = "0803_compare\\masked"
binary_mask_path = "0803_compare\\binary"
diff_set_file = "0803_compare\\diff"

all_file = "0803_compare\\592\\"
all_resize_file = "0803_compare\\128\\"

def resize_img(img, shp):

    scale_percent = 12800/shp # percent of original size
...
for x in range(len(unmasked_path)):
    ...
    for x in range(len(faces2)):
        ...
        for i in range(len(ppoints2)):
            if(b2 < b1 and and_2[ppoints2[i][1]][ppoints2[i][0]] == 255) :
                b2 += 1
                comb_points2.append((int(ppoints2[i][0]), int(ppoints2[i][1])))
        cv2.imshow("comb_img_copy", comb_img_copy)
        cv2.waitKey(0)
    ...
    #cv2.waitKey(0)
    #cv2.destroyAllWindows()
    cv2.imwrite(outputName, line_img)
    cv2.imwrite(noTransName, result)
    ...
    cv2.imwrite(all_resize_file + "masked_landmarks.jpg", resize_img(landmarks_mask2, height))
    cv2.imwrite(all_resize_file + "unmasked_img.jpg", resize_img(img, height))
    cv2.imwrite(all_resize_file + "masked_img.jpg", resize_img(img2, height))
    cv2.imwrite(all_resize_file + "diff_set.jpg", resize_img(diff_set, height))
    cv2.imwrite(all_resize_file + "noTrans.jpg", resize_img(result, height))
    cv2.imwrite(all_resize_file + "binary_mask.jpg", resize_img(binary_mask, height))
    cv2.imwrite(all_resize_file + "diff_rem.jpg", diff_rem)
    '''
    for i in range(0,h):
        for j in range(0,w):
            if img2_new_face[y+i, x+j, 0] == 0 and img2_new_face[y + i, x + j, 1] == 0 and img2_new_face[y + i, x + j, 2] == 0:
