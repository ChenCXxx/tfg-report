#0604explain_mask_comb.py

from cv2 import bitwise_not, subtract
import numpy as np
import cv2
...
MaskImg_path = "0605person5\\test\\image_0105_cloth.jpg"
UnMaskImg_path = "0605person5\\unmasked\\image_0110.jpg"
RefMaskImg_path = "0605person5\\unmasked\\image_0105.jpg"
file_dir = "0605person5\\result"
file_dir_real = "0605person5\\real"
color = (0, 0, 255)
size = 4
...
ref_img = cv2.imread(RefMaskImg_path)
#顯示原始圖片
#filename = NoMaskImg_path[-8:-4]
# output_path = file_dir + "\\" + filename + "_binary" + ".jpg"

truth_mask = np.zeros_like(img)


#diff = ImageChops.difference(img, ref_img)
height, width, channel = img.shape

image_one = Image.open(MaskImg_path)
image_two = Image.open(RefMaskImg_path)
...
'''for i in range(height) :
    for j in range(width) :
        dif = 0
        for k in range(3) :
            if(img[i][j][k] != ref_img[i][j][k]) :
                dif = 1

        if dif == 1 :
            truth_mask[i][j] = (255, 255, 255)
'''
...
ret, mask = cv2.threshold(diff, 254, 255, cv2.THRESH_BINARY)
...
if(flag == True):
    ...
    img_copy = img.copy()
    grabcut_output = grabcut_output.astype(np.float32)
    grabcut_output = np.uint8(grabcut_output)
    grabcut_output_gray = cv2.cvtColor(grabcut_output, cv2.COLOR_BGR2GRAY)
    #print(type(grabcut_output_gray))
    grabcut_img = cv2.bitwise_and(img_copy, img_copy, mask = grabcut_output_gray)
    ...
    img_copy = img.copy()
    binaryImg = binaryImg.astype(np.float32)
    binaryImg = np.uint8(binaryImg)
    regionGrow_output_gray = cv2.cvtColor(binaryImg, cv2.COLOR_BGR2GRAY)
    regionGrow_output_img = cv2.bitwise_and(img_copy, img_copy, mask = regionGrow_output_gray)
    cv2.imshow("regionGrow_output_img", regionGrow_output_img)
    cv2.waitKey(0)
    ...
    for i in range(height):
        for j in range(weight):
            ...
            if x == 255 and y == 255 :
                combImg[i][j] = label
    img_copy = img.copy()
    combImg = np.uint8(combImg)
    combImg_gray = cv2.cvtColor(combImg, cv2.COLOR_BGR2GRAY)
    comb_output_img = cv2.bitwise_and(img_copy, img_copy, mask = combImg_gray)
    cv2.imshow('combine result', comb_output_img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    
    cv2.imwrite(file_dir + "\\" + "grabcut_output.jpg", grabcut_output)
    cv2.imwrite(file_dir + "\\" + "binaryImg.jpg", binaryImg)
    cv2.imwrite(file_dir + "\\" + "dilation_img.jpg", dilation_img)
    cv2.imwrite(file_dir + "\\" + "combImg.jpg", combImg)

    cv2.imwrite(file_dir_real + "\\" + "grabcut_output_real.jpg", grabcut_img)
    cv2.imwrite(file_dir_real + "\\" + "regionGrow_output_ral.jpg", regionGrow_output_img)
    # cv2.imwrite(file_dir_real + "\\" + "dilation_img.jpg", dilation_img)
    cv2.imwrite(file_dir_real + "\\" + "combImg_real.jpg", comb_output_img)
